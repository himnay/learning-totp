...
public class RecoveryCodeRepository {
    ...
    /** Deletes any codes already issued for this app, then inserts a fresh batch. */
    public void replaceAll(String appId, List<String> codes) {
        jdbc.update("DELETE FROM totp_recovery_code WHERE app_id = ?", appId);

        List<Object[]> batchArgs = codes.stream().map(code -> new Object[] {appId, code}).toList();
        jdbc.batchUpdate("INSERT INTO totp_recovery_code (app_id, code) VALUES (?, ?)", batchArgs);
    }
     ...
     * distinguish "wrong code" from "already used" from "unknown app" in its return value —
     * leaking that distinction would help an attacker enumerate valid codes.
     */
    public boolean redeem(String appId, String code) {
        int updated =
                jdbc.update(
                        """
